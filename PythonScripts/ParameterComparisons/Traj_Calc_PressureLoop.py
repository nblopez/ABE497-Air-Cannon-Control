...
for Res_Pressure_psi in np.linspace(10,300, (300-10)):
	...
	if MaxDist >= MaxDist_Check:
		...
		Press_Check = Res_Pressure_psi
		
	#Plot Trajectory
	plt.plot(x_dist, y_dist)

print("Max Distance (m): %f"%MaxDist_Check)
print("Optimal Pressure (psi): %f"%Press_Check)

	

#plt.legend(loc='best')
plt.text(1,1, "Maximum Distance (m): ~%i \nOptimal Pressure (psi): ~%.1f"%(MaxDist_Check,Press_Check),bbox = dict(facecolor='gray', alpha = 0.75))
plt.title("Plot of Trajectories given variation in reservoir pressure")
